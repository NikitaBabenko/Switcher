...
### Removed
- **Auto-correct as you type feature, pulled from v1.0 in favour of shipping a focused first release.** Deleted `extension/content/autocorrect.js` and its test file `extension/lib/autocorrect.test.mjs`, dropped the toggle from `extension/options.html`/`options.js`, removed `autoCorrect` from `extension/config.js` defaults, scrubbed `options_autoCorrect` / `options_autoCorrectHint` keys from all 12 `_locales/<code>/messages.json` files, and removed the feature bullet + comparison-table row + per-locale FAQ entry from the long-form store descriptions (`landing/store/*.md`, `extension/store-listings/*.md`) and SEO landings (`landing/*.md`). Planned to return in v1.1 once polished.

### Changed
- **Replaced broad `host_permissions: ["http://*/*", "https://*/*"]` with on-demand `chrome.scripting.executeScript` injection gated by `activeTab`.** With auto-correct gone, no code path needs persistent page access — every entry point (shortcut, context menu, side panel buttons) is an explicit user gesture that grants `activeTab` on the active tab. `extension/manifest.json` drops `host_permissions` and `content_scripts` entirely; `extension/background.js` gains an `ensureContentInjected(tabId)` helper plus an `ENSURE_CONTENT_INJECTED` message branch; the three content scripts (`content.js`, `content/replace.js`, `content/adapters.js`) wrap their bodies in IIFE-sentinel guards (`globalThis.__switcher_*_loaded`) so re-injection on subsequent gestures is a no-op. Net effect: the install prompt no longer asks for "Read and change all your data on all websites," which removes the slow Chrome Web Store review path. Documented in `extension/PRIVACY.md` (permission table) and `extension/README.md` ("Notes for review").
...
- 223 Node tests (detector, config, adapters, replace, content, package, i18n).
